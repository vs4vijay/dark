...
let fluid_single_click_on_token_in_selected_handler_places_cursor (m : model) :
    testResult =
    let focusedPass =
      match m.currentPage with
      | FocusedHandler (tlid, _) when tlid = TLID.fromString "453209170" ->
          pass
      | _ ->
          fail "handler is not focused"
    in
    ...
    in
    Result.combine [focusedPass; browserCursorPass; cursorPass] |> Result.map (fun _ -> ())
...
let trigger (test_name : string) : integrationTestState =
  ...
  IntegrationTestExpectation
    ...
    | "fluid_single_click_on_token_in_deselected_handler_focuses" ->
        fluid_single_click_on_token_in_deselected_handler_focuses
    | "fluid_single_click_on_token_in_selected_handler_places_cursor" ->
        fluid_single_click_on_token_in_selected_handler_places_cursor
